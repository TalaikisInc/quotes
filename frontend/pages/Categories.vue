<template>
  <div>
    ...
    <Row>
      <Col :span="20">
        <h1>Quotes by author<span v-if="page > 0">, page {{ page }}</span></h1>
      </Col>
    </Row>
    <Row v-for="(chunk, index) in chunkCats" :key="index">
      <Col :span="5" v-for="(cat, i) in chunk" :key="i">
        <h3><a :href="baseUrl + keyword + '/' + cat.slug + '/'">{{ cat.title }} quotes</a> [{{ cat.post_count }}]</h3>
      </Col>
...
export default {
  ...
  computed: {
    ...
    calcPages () {
      const pages = Math.floor(this.categories[0].total_cats / this.itemsPerPage)
      return pages
    }
  },
  head () {
